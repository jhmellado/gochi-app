...
const ImagenPerfil = (props) => {
  ...
  const actualizarFotoPerfil = (event) => {
    event.preventDefault();
    console.log(event);
    if (!imagenInput.trim()) {
      setError("Ingrese una imagen por favor");
      return;
    }
  ...
  const imagenPrevia = (imagen) => {
    if (imagen.files.length !== 0) {
      ...
      listRef
          ...
          if (res.items.length > 0) {
            res.items.forEach((itemRef) => {
              var desertRef = storageRef.child(
                "vista_previa/" + auth.currentUser.uid + "/" + itemRef.name
              );
              desertRef
                .delete()
                .then(() => {
        ...
        .then(() => {
          var imagenRef = storageRef
            .child(
              "vista_previa/" + auth.currentUser.uid + "/" + imagen.files.name
            )
            .put(imagen.files[0]);
          imagenRef.on(
            "state_changed",
      ...
      console.log("no ha seleccionado imagen");
      setImagenInput("");
    }
  };
      ...
      <div className="container py-5">
        <div className="d-flex row justify-content-center">
          <div className="col-12 col-md-8 px-0">
            ...
            <form onSubmit={actualizarFotoPerfil}>
              <div className="container mb-4">
                <div className="d-flex row align-items-end">
                  <div className="col">
                    <label
                    ...
                    <input
                      ...
                      id="file_form"
                      accept=".jpg,.jpeg,.png"
                      onChange={(event) => imagenPrevia(event.target)}
                    />
                  </div>
                </div>
              </div>
